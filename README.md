<p align="center">
  <img src="public/logo.svg" width="200" alt="El LibreDB Studio Logo" />
</p>

<p align="center">
  <img src="el-libredb.png" alt="El LibreDB Studio" width="320" />
</p>

<h1 align="center">El LibreDB Studio</h1>
...
<p align="center">
  <img src="public/screenshots/hero-demo.gif" alt="Opening a table, running a join, charting the result and reading the ER diagram in El LibreDB Studio" width="100%" />
</p>
...
> **Try El LibreDB Studio instantly without installation!**
...
El LibreDB Studio goes the other way. It deploys next to the data: a container, a Helm chart, an operator, a one-click template on your PaaS, or `npm i @libredb/studio` inside your own product. Nothing has to face outward.
...
- **A work kernel beneath the agent.** Runs are admitted through open, registry-backed task,
  occupation, and capability identifiers rather than a new isolated agent per job. The existing
  durable run service and database policy envelope remain the execution boundary: the work kernel
  checks task assignment, occupation capability authority, risk, role, mode, and approval before
  the investigation loop can reach the existing operation-level policy gate.
...
## Getting Started
  ...
  Run El LibreDB Studio with a single command — no clone, no install, no build:
...
El LibreDB Studio has a comprehensive test suite with **3,000+ unit/integration tests** and **32 E2E tests** across 6 layers, with **100% line coverage** enforced by CI (`bun run coverage:check`).
...
Deploy your own instance of El LibreDB Studio with a single click on DigitalOcean, Koyeb, Render, Railway, Sealos, CapRover, or Dokploy:
...
>
> **CapRover:** open your CapRover dashboard → **Apps → One-Click Apps/Databases**, search for **El LibreDB Studio**, and deploy.
>
...
>
> **Cosmos:** install in one click from the [Cosmos](https://cosmos-cloud.io) Marketplace — search for **El LibreDB Studio**. Cosmos auto-generates secrets, provisions a persistent SQLite volume, and serves the app behind its SmartShield reverse proxy. See [`deploy/cosmos/`](deploy/cosmos/).
>
> **Dokploy:** install in one click from the [Dokploy template catalog](https://templates.dokploy.com) — in your Dokploy dashboard, **Create Service → Template**, search for **El LibreDB Studio**, and deploy. Dokploy auto-generates `ADMIN_PASSWORD`, `USER_PASSWORD`, and `JWT_SECRET`, and persists connections on a SQLite volume behind Traefik. See [`deploy/dokploy/`](deploy/dokploy/).
...
El LibreDB Studio is available as a one-click [Railway](https://railway.com) template.
...
El LibreDB Studio is published in the official [CapRover One-Click Apps](https://github.com/caprover/one-click-apps/blob/master/public/v4/apps/libredb-studio.yml) catalog:

1. **Open your CapRover dashboard** → **Apps → One-Click Apps/Databases**
2. **Search** for **El LibreDB Studio**
...
El LibreDB Studio is listed in the official
...
**Templates**, search **El LibreDB Studio**, fill in the credentials / `JWT_SECRET`,
...
**El LibreDB Studio**, and install. Cosmos auto-generates the credentials and
...
El LibreDB Studio includes a `render.yaml` Blueprint for one-click deployment:
